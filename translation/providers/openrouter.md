...
description: "Otomatik yönlendirme ve maliyet optimizasyonu özellikleri ile OpenRouter'ın birleşik API ağ geçidi üzerinden 100'den fazla LLM sağlayıcısına erişin"
---

# OpenRouter

[OpenRouter](https://openrouter.ai/), OpenAI, Meta, Perplexity ve diğerlerinden gelen modeller dahil olmak üzere çeşitli LLM API'lerine erişmek için birleşik bir arayüz sağlar. OpenAI API formatını takip eder - temel API ayrıntıları için [OpenAI sağlayıcı belgelerimize](/docs/providers/openai/) bakın.

## Kurulum

1. [OpenRouter](https://openrouter.ai/) adresinden API anahtarınızı alın.
2. `OPENROUTER_API_KEY` ortam değişkenini ayarlayın veya yapılandırmanızda `apiKey` belirtin.

## Kullanılabilir Modeller

Son çıkanlar:

| **Model Kimliği (ID)**                                                                                      | **Bağlam (tokens)** |
| ---------------------------------------------------------------------------------------------------------- | ------------------- |
...
300'den fazla modelin tam listesi ve ayrıntılı fiyatlandırma için [OpenRouter Modelleri](https://openrouter.ai/models) sayfasını ziyaret edin.
...
- Tek bir API üzerinden 300'den fazla modele erişim
- Değerlendirmelerinizde ücretsiz ve ücretli modelleri karıştırın
- Metin ve çok modlu (görme) modelleri desteği
- OpenAI API formatı ile uyumlu
- Kullandıkça öde fiyatlandırması

## Düşünme/Akıl Yürütme Modelleri

Gemini 2.5 Pro gibi bazı modeller yanıtlarına düşünme (thinking) tokenlarını dahil eder. Bunların gösterilip gösterilmeyeceğini `showThinking` parametresini kullanarak kontrol edebilirsiniz:

```yaml title="promptfooconfig.yaml"
providers:
  - id: openrouter:google/gemini-2.5-pro
    config:
      showThinking: false # Düşünme içeriğini çıktıdan gizle (varsayılan: true)
```

`showThinking` doğru (varsayılan) olduğunda, çıktı düşünme içeriğini içerir:

```
Thinking: <akıl yürütme süreci>
