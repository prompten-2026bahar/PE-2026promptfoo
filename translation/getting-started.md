...
---
title: Başlarken
description: İlk promptfoo yapılandırma dosyanızı nasıl kuracağınızı, promptları nasıl oluşturacağınızı, sağlayıcıları nasıl yapılandıracağınızı ve ilk LLM değerlendirmenizi nasıl çalıştıracağınızı öğrenin.
...
[Promptfoo'yu kurduktan](/docs/installation) sonra, ilk yapılandırma dosyanızı birkaç şekilde kurabilirsiniz:
...
![Promptfoo Web UI değerlendirme sonuçlarını gösteriyor](/img/docs/custom-example-view.png)
...
tests:
  ...
  - vars:
      riddle: >-
        Anahtarım var ama kilidim yok. Alanım var ama odam yok. Girebilirsin ama
...
Yeni oluşturulan dizine gidin ve `npx promptfoo@latest eval` veya `promptfoo eval` çalıştırın. Ayrıca komut satırından parametreleri doğrudan geçersiz kılabilirsiniz.
...
- Farklı sıcaklıklarla modelleri karşılaştırın ([GPT sıcaklık karşılaştırması](https://github.com/promptfoo/promptfoo/tree/main/examples/gpt-4o-temperature-comparison) bkz)
- Llama ile GPT'yi karşılaştırın ([Llama vs GPT benchmark](/docs/guides/compare-llama2-vs-gpt) bkz)
- LangChain ile Retrieval-Augmented Generation (RAG) ile normal GPT-4'ü karşılaştırın ([LangChain örneği](/docs/configuration/testing-llm-chains) bkz)
